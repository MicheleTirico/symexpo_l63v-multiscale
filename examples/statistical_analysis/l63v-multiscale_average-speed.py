...
path_mat_travel_distance=pathOutputDir+prefix+"_mat_av-sp.csv"
path_sym=pathOutputDir+"l63v-multiscale_000000_235959_traf_trajectoires.csv"
path_sym_travel_distance=pathOutputDir+prefix+"_sym_av-sp.csv"
...
logger=logger.Logger(storeLog=True,initStore=True,pathLog=pathOutputDir+prefix+"_log_average-speed.md")
hf.setLogger(logger=logger)
logger.setDisplay(True,True,True,True)
logger.storeLocal(False)
cwd=hf.getDefCwd()
logger.log(cl=None,method=None,message="start average-speed")

run_mat,run_sym,run_fig_hist=True,True,True
...
if run_mat:
    ...
    df_mat["av_sp_mps"]=df_mat["traveled_distance"]/df_mat["trav_time"]
...
if run_sym:
    ...
    df_sym["av_sp_mps"]=df_sym["traveled_distance"]/df_sym["trav_time"]
...
if run_fig_hist:
    ...
    df1_mat=df_mat[["trip_id","av_sp_mps"]]
    df1_mat.insert(loc=2,column="model",value="MATSim")
    df1_sym=df_sym[["trip_id","av_sp_mps"]]
    df1_sym.insert(loc=2,column="model",value="Symuvia")
    df1_sym=df1_sym[df1_sym["av_sp_mps"]<df1_sym["av_sp_mps"].quantile(quantile)]
    df1_mat=df1_mat[df1_mat["av_sp_mps"]<df1_mat["av_sp_mps"].quantile(quantile)]

    df1=pd.concat([df1_mat,df1_sym]).reset_index()
    df1=df1[df1["av_sp_mps"]>=0]

    fig, ax = plt.subplots()
    ax.set(xlabel="average speed [m/s]", ylabel="probability")
    sns.histplot(data=df1, x="av_sp_mps",hue="model", multiple="stack",log_scale=log_scale,alpha=alpha,bins=bins,palette=[palette[position_color[0]],palette[position_color[1]]],stat="probability")
    tools.saveFig(fig=fig,pathSave=cwd+pathOutputFig+"{}_{}_{}_quant-{}_log-{}.jpg".format(prefix,"av-sp","hist",quantile,log_scale),logger=logger)
    plt.close()
